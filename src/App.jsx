...
const C = {
  ...
  txt:"#2D1810", muted:"#6B5740", light:"#7A6B5D",
...
export default function App() {
  ...
  return (
    <div style={{ fontFamily:"'Plus Jakarta Sans',sans-serif", background:C.bg, minHeight:"100vh", color:C.txt }}>
      ...
      {banner && dist.active && (
        <div style={{ position:"fixed",top:0,left:0,right:0,zIndex:150,background:`linear-gradient(135deg,${C.accent},${C.accentL})`,padding:mob?"8px 12px":"10px 48px",display:"flex",alignItems:"center",justifyContent:"center",gap:mob?6:16,animation:"bannerSlide 0.5s ease",flexWrap:"wrap" }}>
          ...
          <div style={{ display:"flex",gap:6 }}>
            <button onClick={() => setBanner(false)} style={{ fontSize:13,fontWeight:700,color:C.accent,background:"#FFF",border:"none",padding:"4px 12px",borderRadius:100,cursor:"pointer" }}>Yes!</button>
            <button onClick={() => { setDdOpen(true); setBanner(false); }} style={{ fontSize:13,fontWeight:600,color:"rgba(255,255,255,0.9)",background:"rgba(255,255,255,0.15)",border:"1px solid rgba(255,255,255,0.3)",padding:"4px 12px",borderRadius:100,cursor:"pointer" }}>Change</button>
          </div>
      ...
      <header style={{ position:"fixed",top:topOff,left:0,right:0,zIndex:100,padding:mob?"10px 12px":"14px 48px",display:"flex",alignItems:"center",justifyContent:"space-between",background:"rgba(251,246,239,0.92)",backdropFilter:"blur(16px)",borderBottom:`1px solid ${C.border}`,transition:"top 0.3s" }}>
        ...
        <div style={{ display:"flex",alignItems:"center",gap:mob?6:12 }}>
          ...
          {logged ? (
            ...
            <>
              {mob && <button onClick={() => nav("catalog")} style={{ fontSize:13,fontWeight:600,color:C.muted,background:"none",border:"none",cursor:"pointer",padding:"6px 8px" }}>Catalog</button>}
      ...
      {showRate && <RatingModal mob={mob} delivery={showRate} onClose={() => setShowRate(null)} />}

      {/* District picker modal */}
      {ddOpen && (
        <div style={{ position:"fixed",inset:0,zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:mob?16:24,background:"rgba(45,24,16,0.45)",backdropFilter:"blur(10px)" }} onClick={() => setDdOpen(false)}>
          <div style={{ background:"#FFF",borderRadius:24,padding:mob?"24px 20px":"36px 40px",width:"100%",maxWidth:520,maxHeight:"80vh",animation:"modalIn 0.3s ease",position:"relative",display:"flex",flexDirection:"column",boxShadow:"0 24px 80px rgba(45,24,16,0.2)",overflow:"hidden" }} onClick={e=>e.stopPropagation()}>
            <button onClick={() => setDdOpen(false)} style={{ position:"absolute",top:16,right:16,background:"rgba(0,0,0,0.04)",border:"none",cursor:"pointer",color:C.muted,width:32,height:32,borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",zIndex:1 }}><Ic.X /></button>
            <div style={{ textAlign:"center",marginBottom:24,flexShrink:0 }}>
              <h2 style={{ fontFamily:"'Fraunces',serif",fontSize:mob?22:28,fontWeight:700,marginBottom:8 }}>Choose your area</h2>
              <p style={{ fontSize:15,color:C.muted }}>Select a neighborhood or type your address</p>
            </div>
            <div style={{ position:"relative",marginBottom:20,flexShrink:0 }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={C.muted} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ position:"absolute",left:16,top:"50%",transform:"translateY(-50%)" }}><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
              <input
                type="text"
                placeholder="Type your address to find your area..."
                style={{ width:"100%",padding:"14px 16px 14px 44px",borderRadius:14,border:`2px solid ${C.border}`,fontSize:15,fontFamily:"'Plus Jakarta Sans',sans-serif",outline:"none",transition:"border-color 0.2s" }}
                onFocus={e=>e.target.style.borderColor=C.accent}
                onBlur={e=>e.target.style.borderColor=C.border}
                onChange={e => { console.log("[WeBake Analytics] Location search:", e.target.value); }}
              />
            </div>
            <div style={{ fontSize:12,fontWeight:600,color:C.muted,textTransform:"uppercase",letterSpacing:1.5,marginBottom:10,paddingLeft:4,flexShrink:0 }}>Neighborhoods</div>
            <div style={{ flex:1,overflowY:"auto",minHeight:0,display:"flex",flexDirection:"column",gap:8 }}>
              {dists.map((d, i) => (
                <div key={i} onClick={() => { setDist(d); setDdOpen(false); setBanner(true); nav("home"); }} style={{ padding:"16px 18px",borderRadius:16,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"space-between",transition:"all 0.2s",border:`1.5px solid ${C.border}`,background:"#FFF",flexShrink:0 }}
                  onMouseEnter={e => { e.currentTarget.style.background="rgba(199,91,43,0.04)"; e.currentTarget.style.borderColor=C.bAccent; }} onMouseLeave={e => { e.currentTarget.style.background="#FFF"; e.currentTarget.style.borderColor=C.border; }}>
                  <div><div style={{ fontSize:16,fontWeight:600 }}>{d.name}</div><div style={{ fontSize:14,color:C.muted,marginTop:3 }}>{d.city}</div></div>
                  <span style={{ fontSize:13,fontWeight:700,color:d.active?C.ok:C.muted,background:d.active?"rgba(61,139,94,0.1)":"rgba(0,0,0,0.04)",padding:"6px 16px",borderRadius:100 }}>{d.active?"Active":"Waitlist"}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div style={{ paddingTop: topOff + (mob ? 52 : 60) }}>
        {!dist.active ? <WaitlistPage dist={dist} {...ctx} /> :
         ...
         pg === "history" ? <HistoryPage onRate={(del) => setShowRate(del)} {...ctx} /> :
...
function HomePage({ dist, mob, tab, pad, nav, goSched }) {
  ...
  return (
    <div>
          ...
          <div>
            ...
            <div style={{ display:"flex",gap:8,flexWrap:"wrap",marginTop:mob?16:28,animation:"fadeUp 0.6s ease 1s both" }}>
              {["✓ Cancel anytime","🎉 50% off first week","🔒 Stripe secure"].map((t,i) => (
                <div key={i} style={{ fontSize:12,fontWeight:600,color:C.light,background:"rgba(139,115,85,0.06)",padding:"5px 12px",borderRadius:100 }}>{t}</div>
              ))}
            </div>
          </div>
          ...
          <div style={{ position:"relative",animation:"scaleIn 0.8s ease 0.4s both" }}>
            ...
            {!mob && <>
              <div style={{ position:"absolute",bottom:-20,left:-32,background:"#FFF",padding:"14px 18px",borderRadius:18,boxShadow:"0 8px 32px rgba(0,0,0,0.08)",display:"flex",alignItems:"center",gap:10,animation:"float1 3s ease infinite" }}>
                ...
                <div><div style={{ fontSize:13,fontWeight:600 }}>Tomorrow, 6:30 AM</div><div style={{ fontSize:13,color:C.light,marginTop:1 }}>Next delivery</div></div>
              </div>
              <div style={{ position:"absolute",top:24,right:-28,background:"#FFF",padding:"10px 10px 14px",borderRadius:16,boxShadow:"0 8px 32px rgba(0,0,0,0.08)",animation:"float2 4s ease 1s infinite",width:150 }}>
                ...
                <div style={{ fontSize:13,letterSpacing:1 }}>⭐⭐⭐⭐⭐</div>
                <div style={{ fontSize:12,fontWeight:600,marginTop:2 }}>"Best bread ever!"</div>
                <div style={{ fontSize:11,color:C.light,marginTop:1 }}>— Sarah M.</div>
              </div>
            </>}
          </div>
        </div>
      ...
      {/* HOW IT WORKS */}
      <section style={{ padding:mob?"60px 16px 80px":"100px 48px 120px",background:C.cream,position:"relative" }}>
        <div style={{ maxWidth:1100,margin:"0 auto" }}>
          <div style={{ textAlign:"center",marginBottom:mob?40:64 }}>
            <div style={{ fontSize:13,fontWeight:700,letterSpacing:3,textTransform:"uppercase",color:C.accent,marginBottom:14 }}>How It Works</div>
            <h2 style={{ fontFamily:"'Fraunces',serif",fontSize:mob?30:48,fontWeight:700,color:C.txt,marginBottom:14,lineHeight:1.1 }}>Bread on autopilot</h2>
            <p style={{ fontSize:mob?15:18,color:C.muted,maxWidth:520,margin:"0 auto",lineHeight:1.6 }}>Four simple steps to fresh bread at your door every morning.</p>
          </div>

          <div style={{ display:"grid",gridTemplateColumns:mob?"1fr":(tab?"1fr 1fr":"repeat(4,1fr)"),gap:mob?20:24 }}>
            {[
              { num:"01",title:"Choose Your Bread",desc:"Browse 10+ varieties baked from scratch with the finest ingredients.",
                icon:<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={C.accent} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg> },
              { num:"02",title:"Set Your Schedule",desc:"Pick delivery days & quantities. Fully flexible — change anytime.",
                icon:<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={C.accent} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg> },
              { num:"03",title:"We Bake & Deliver",desc:"Every morning we bake it fresh and deliver hot to your door before 9 AM.",
                icon:<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={C.accent} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="3" width="15" height="13" rx="2"/><polyline points="16 8 20 8 23 11 23 16 16 16 16 8"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg> },
              { num:"04",title:"Pay Weekly",desc:"Charged only for what we delivered. No commitments, cancel anytime.",
                icon:<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={C.accent} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="4" width="22" height="16" rx="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg> },
            ].map((step, i) => (
              <div key={i} style={{
                background:"#FFF",borderRadius:mob?18:22,padding:mob?"28px 20px":"32px 28px",
                border:`1px solid ${C.border}`,transition:"all 0.3s",position:"relative",
              }}
                onMouseEnter={e => { if(!mob){ e.currentTarget.style.transform="translateY(-6px)"; e.currentTarget.style.boxShadow="0 16px 48px rgba(199,91,43,0.1)"; e.currentTarget.style.borderColor=C.bAccent; }}}
              ...
              >
                <div style={{ display:"flex",alignItems:"center",gap:14,marginBottom:mob?16:20 }}>
                  <div style={{ width:52,height:52,borderRadius:14,background:"rgba(199,91,43,0.08)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0 }}>{step.icon}</div>
                  <span style={{ fontFamily:"'Fraunces',serif",fontSize:14,fontWeight:700,color:C.accent,letterSpacing:1 }}>{step.num}</span>
                </div>
                <h3 style={{ fontFamily:"'Fraunces',serif",fontSize:mob?19:21,fontWeight:700,color:C.txt,marginBottom:mob?8:10,lineHeight:1.2 }}>{step.title}</h3>
                <p style={{ fontSize:mob?14:15,lineHeight:1.65,color:C.muted }}>{step.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding:mob?"60px 16px":"100px 48px" }}>
        <div style={{ textAlign:"center",marginBottom:mob?32:56 }}>
          <div style={{ fontSize:14,fontWeight:600,letterSpacing:3,textTransform:"uppercase",color:C.accent,marginBottom:12 }}>Our Selection</div>
  ...
  return (
    <div onClick={onClick} style={{ background:"#FFF",borderRadius:mob?14:20,overflow:"hidden",border:`1px solid ${C.border}`,transition:"all 0.3s",cursor:"pointer" }}
      ...
      <div style={{ padding:mob?"12px":"20px 20px 24px" }}>
        ...
        {!mob && <p style={{ fontSize:13,color:C.light }}>Allergens: {p.allergens}</p>}
  ...
  return (
    <div style={{ maxWidth:1200,margin:"0 auto",padding:`40px ${mob?"16px":"48px"}` }}>
      <div style={{ marginBottom:mob?24:48 }}>
        <div style={{ fontSize:14,fontWeight:600,letterSpacing:3,textTransform:"uppercase",color:C.accent,marginBottom:8 }}>Our Bread</div>
  ...
  return (
    <div style={{ maxWidth:1200,margin:"0 auto",padding:`${mob?20:40}px ${mob?"16px":"48px"} ${mob?"120px":"40px"}` }}>
      <div style={{ marginBottom:mob?20:40 }}>
        <div style={{ fontSize:14,fontWeight:600,letterSpacing:3,textTransform:"uppercase",color:C.accent,marginBottom:8 }}>Core Feature</div>
                      ...
                      <div style={{ gridColumn:`1 / -1`,background:"rgba(199,91,43,0.03)",border:`2px solid ${C.accent}`,borderRadius:mob?12:16,padding:mob?10:14,animation:"slideDown 0.25s ease" }}>
                        ...
                        <div style={{ display:"flex",flexDirection:mob?"column":"row",alignItems:mob?"stretch":"center",gap:mob?8:12 }}>
                          <div style={{ display:"flex",alignItems:"center",gap:5 }}>
                            <span style={{ fontSize:14,fontWeight:500,color:C.muted }}>Qty:</span>
                          ...
                          <div style={{ display:"flex",alignItems:"center",gap:mob?4:5,flexWrap:"wrap" }}>
                            <span style={{ fontSize:14,fontWeight:500,color:C.muted }}>Days:</span>
                            {DAYS.map(d => (
                              <button key={d} onClick={() => setAddDays(addDays.includes(d)?addDays.filter(x=>x!==d):[...addDays,d])} style={{
                                width:mob?34:36,height:mob?34:36,borderRadius:8,fontSize:12,fontWeight:600,
                          ...
                          <button onClick={addItem} disabled={addDays.length===0} style={{
                            fontSize:14,fontWeight:700,color:"#FFF",marginLeft:mob?0:"auto",
                ...
                {items.map((item, idx) => (
                  <div key={idx} style={{ background:"#FFF",borderRadius:14,padding:14,border:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:10 }}>
                    ...
                    <div style={{ flex:1,minWidth:0 }}>
                      ...
                      <div style={{ fontSize:14,color:C.light,marginTop:2 }}>{item.days.join(", ")}</div>
                    </div>
              ...
              <div style={{ background:"#FFF",borderRadius:20,overflow:"hidden",border:`1px solid ${C.border}` }}>
                <table style={{ width:"100%",borderCollapse:"collapse" }}>
                  <thead><tr style={{ borderBottom:`1px solid ${C.border}` }}>
                    ...
                    {DAYS.map(d => <th key={d} style={{ padding:"16px 6px",textAlign:"center",fontSize:14,fontWeight:600,color:C.muted,width:44 }}>{d}</th>)}
                    <th style={{ width:36 }} />
                  </tr></thead>
                    ...
                    <tr key={idx} style={{ borderBottom:`1px solid ${C.border}` }}>
                      <td style={{ padding:"12px 20px" }}>
                        <div style={{ display:"flex",alignItems:"center",gap:10 }}>
                          ...
                          <div><div style={{ fontSize:14,fontWeight:600 }}>{item.product.name}</div><div style={{ fontSize:14,color:C.light }}>×{item.qty} · ${item.product.price.toFixed(2)} ea</div></div>
                        </div>
                      </td>
        ...
        {mob ? (
          <>
            ...
            <div style={{ position:"fixed",bottom:0,left:0,right:0,background:"#FFF",borderTop:`1px solid ${C.border}`,padding:"12px 16px",zIndex:90,display:"flex",alignItems:"center",justifyContent:"space-between",boxShadow:"0 -4px 20px rgba(0,0,0,0.06)" }}>
              <div>
                <div style={{ fontSize:13,color:C.muted }}>Weekly Total</div>
                <div style={{ fontSize:20,fontWeight:700,color:C.accent }}>${grandTotal.toFixed(2)}</div>
              </div>
              <div style={{ display:"flex",gap:8,alignItems:"center" }}>
                <button onClick={() => setShowSummary(true)} style={{ fontSize:14,fontWeight:600,color:C.accent,background:"rgba(199,91,43,0.08)",border:"none",padding:"10px 16px",borderRadius:100,cursor:"pointer" }}>Details</button>
  ...
  return (
    <>
      ...
      <div style={{ display:"flex",alignItems:"center",gap:10,padding:"12px 14px",borderRadius:14,background:"rgba(199,91,43,0.04)",border:`1.5px solid ${C.bAccent}`,marginBottom:20 }}>
        ...
        <div style={{ flex:1 }}>
          <label style={{ fontSize:13,fontWeight:600,color:C.accent,display:"block",marginBottom:3 }}>Choose your delivery time</label>
      ...
      <div style={{ background:"rgba(199,91,43,0.06)",borderRadius:10,padding:"8px 12px",fontSize:14,fontWeight:600,color:C.accent,textAlign:"center",marginBottom:20 }}>🎉 50% off your first week!</div>
        ...
        {items.length>0?(
          <div style={{ overflowX:"auto" }}>
            <table style={{ width:"100%",borderCollapse:"collapse",minWidth:500 }}>
              <thead><tr>{["Item",...DAYS].map((h,i)=><th key={i} style={{ padding:"8px 6px",textAlign:i===0?"left":"center",fontSize:14,fontWeight:600,color:C.light }}>{h}</th>)}</tr></thead>
  ...
  return (
    <div style={{ background:"#FFF",borderRadius:mob?14:20,padding:mob?16:28,marginBottom:mob?12:20,border:`1px solid ${C.border}` }}>
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12 }}>
        ...
        {action && <button onClick={onAction} style={{ fontSize:14,fontWeight:600,color:C.accent,background:"none",border:"none",cursor:"pointer",display:"flex",alignItems:"center",gap:4 }}><Ic.Edit />{action}</button>}
      </div>
      {children}
    </div>
  );
...
function HistoryPage({ onRate, mob }) {
  const dels = [
    {date:"Feb 24",items:"2× Baguette, 1× Ciabatta",total:11.50,rating:5,img:IMG_BAGUETTE,courier:"Alex",
      products:[{name:"Baguette",qty:2,img:IMG_BAGUETTE},{name:"Ciabatta",qty:1,img:IMG_CIABATTA}]},
    {date:"Feb 23",items:"2× Baguette",total:9.00,rating:4,img:IMG_SOURDOUGH,courier:"Alex",
      products:[{name:"Baguette",qty:2,img:IMG_BAGUETTE}]},
    {date:"Feb 22",items:"2× Baguette, 1× Flatbread",total:13.00,rating:null,img:IMG_FLATBREAD,courier:"Alex",
      products:[{name:"Baguette",qty:2,img:IMG_BAGUETTE},{name:"Flatbread",qty:1,img:IMG_FLATBREAD}]},
    {date:"Feb 21",items:"2× Baguette",total:9.00,rating:5,img:IMG_BAGUETTE,courier:"Alex",
      products:[{name:"Baguette",qty:2,img:IMG_BAGUETTE}]},
    {date:"Feb 20",items:"2× Baguette, 1× Ciabatta",total:11.50,rating:4,img:IMG_CIABATTA,courier:"Alex",
      products:[{name:"Baguette",qty:2,img:IMG_BAGUETTE},{name:"Ciabatta",qty:1,img:IMG_CIABATTA}]},
  ];
  return (
    <div style={{ maxWidth:900,margin:"0 auto",padding:`40px ${mob?"16px":"48px"}` }}>
        ...
        {dels.map((d,i) => (
          <div key={i} style={{ display:"flex",alignItems:"center",justifyContent:"space-between",background:"#FFF",borderRadius:mob?12:16,padding:mob?"12px 14px":"20px 28px",border:`1px solid ${C.border}`,flexWrap:mob?"wrap":"nowrap",gap:mob?8:16 }}>
            ...
            <div style={{ display:"flex",alignItems:"center",gap:mob?10:24 }}>
              ...
              :<button onClick={() => onRate(d)} style={{ fontSize:13,fontWeight:600,color:C.accent,background:"rgba(199,91,43,0.08)",border:"none",padding:"5px 14px",borderRadius:100,cursor:"pointer" }}>Rate</button>}
            </div>
          </div>
        ))}
      </div>
        ...
        <div key={i} style={{ background:"#FFF",borderRadius:mob?12:16,overflow:"hidden",border:`1px solid ${C.border}`,marginBottom:12 }}>
          <div onClick={() => setExp(exp===i?null:i)} style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:mob?"14px 16px":"20px 28px",cursor:"pointer",gap:8 }}>
            <div style={{ display:"flex",alignItems:"center",gap:mob?8:16,minWidth:0 }}>
              {!mob && <Ic.Card />}
              <div style={{ minWidth:0 }}><div style={{ fontSize:mob?13:15,fontWeight:600 }}>Week of {w.period}</div>{w.first && <span style={{ fontSize:13,color:C.accent,fontWeight:600 }}>50% off!</span>}</div>
            </div>
            <div style={{ display:"flex",alignItems:"center",gap:mob?6:12,flexShrink:0 }}>
              ...
              <span style={{ fontSize:13,fontWeight:600,color:C.ok,background:"rgba(61,139,94,0.08)",padding:"3px 8px",borderRadius:100 }}>Paid</span>
    ...
    <div style={{ maxWidth:700,margin:"0 auto",padding:`60px ${mob?"16px":"48px"}`,textAlign:"center" }}>
      ...
      {!ok?(
        <div style={{ background:"#FFF",borderRadius:mob?16:24,padding:mob?20:36,textAlign:"left",border:`1px solid ${C.border}` }}>
          ...
          <Field label="Your address *" placeholder="e.g. 1600 Pennsylvania Ave, Washington DC" mob={mob} />
      ...
      <div style={{ background:"#FFF",borderRadius:mob?20:28,padding:mob?24:40,width:mob?"calc(100% - 32px)":420,animation:"modalIn 0.3s ease",position:"relative",maxHeight:"90vh",overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
        ...
        {!sent ? (
          <>
            ...
            <div style={{ textAlign:"center",marginTop:16,fontSize:14,color:C.light }}>No password needed.</div>
          </>
        ) : (
...
function StarRow({ value, onChange, size=24 }) {
  return (
    <div style={{ display:"flex",gap:4 }}>
      {[1,2,3,4,5].map(s => (
        <button key={s} onClick={() => onChange(s)} style={{ background:"none",border:"none",cursor:"pointer",padding:1,transform:s<=value?"scale(1.15)":"",transition:"transform 0.2s" }}>
          <svg width={size} height={size} viewBox="0 0 24 24" fill={s<=value?"#E8A33A":"none"} stroke={s<=value?"#E8A33A":"#ccc"} strokeWidth="2"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
        </button>
      ))}
    </div>
  );
...
function RatingModal({ mob, delivery, onClose }) {
  const products = delivery?.products || [];
  const totalSteps = 1 + products.length; // courier + each product
  const [step, setStep] = useState(0); // 0 = courier, 1..N = products
  const [courierRating, setCourierRating] = useState(0);
  const [productRatings, setProductRatings] = useState(products.map(() => 0));
  const [done, setDone] = useState(false);

  const setProductRating = (idx, val) => {
    const next = [...productRatings];
    next[idx] = val;
    setProductRatings(next);
  };

  const currentRating = step === 0 ? courierRating : productRatings[step - 1];
  const isLast = step === totalSteps - 1;

  const handleNext = () => {
    if (isLast) { setDone(true); } else { setStep(step + 1); }
  };

  const w = mob ? "calc(100% - 32px)" : 440;

  if (done) {
    return (
      <Overlay onClose={onClose}>
        <div style={{ background:"#FFF",borderRadius:mob?20:24,padding:mob?24:36,width:w,textAlign:"center",animation:"modalIn 0.3s ease" }} onClick={e=>e.stopPropagation()}>
          <div style={{ fontSize:44,marginBottom:10 }}>🙏</div>
          <h3 style={{ fontFamily:"'Fraunces',serif",fontSize:18,fontWeight:600,marginBottom:6 }}>Thank you for your feedback!</h3>
          <p style={{ fontSize:14,color:C.muted,marginBottom:20 }}>Your ratings help us improve</p>
          <button onClick={onClose} style={{ fontSize:14,fontWeight:600,color:C.accent,background:"rgba(199,91,43,0.08)",border:"none",padding:"10px 24px",borderRadius:100,cursor:"pointer" }}>Done</button>
        </div>
      </Overlay>
    );
  }
  ...
  return (
    <Overlay onClose={onClose}>
      <div style={{ background:"#FFF",borderRadius:mob?20:24,padding:mob?24:36,width:w,animation:"modalIn 0.3s ease",position:"relative" }} onClick={e=>e.stopPropagation()}>
        {/* Progress dots */}
        <div style={{ display:"flex",justifyContent:"center",gap:6,marginBottom:20 }}>
          {Array.from({length:totalSteps}).map((_,i) => (
            <div key={i} style={{ width:i===step?20:8,height:8,borderRadius:4,background:i<=step?C.accent:"rgba(0,0,0,0.08)",transition:"all 0.3s" }} />
          ))}
        </div>
        ...
        {step === 0 ? (
          ...
          <div style={{ textAlign:"center" }}>
            <div style={{ width:56,height:56,borderRadius:"50%",background:`linear-gradient(135deg,${C.accent},${C.accentL})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,color:"#FFF",margin:"0 auto 12px" }}>🚴</div>
            <h2 style={{ fontFamily:"'Fraunces',serif",fontSize:mob?18:22,fontWeight:600,marginBottom:4 }}>Rate your courier</h2>
            <p style={{ fontSize:14,color:C.muted,marginBottom:4 }}>{delivery?.courier || "Alex"} · {delivery?.date}</p>
            <p style={{ fontSize:13,color:C.light,marginBottom:20 }}>Was the delivery on time and handled with care?</p>
            <div style={{ display:"flex",justifyContent:"center",marginBottom:24 }}>
              <StarRow value={courierRating} onChange={setCourierRating} size={mob?28:32} />
            </div>
          </div>
        ) : (
          /* Product rating */
          <div style={{ textAlign:"center" }}>
            <div style={{ width:64,height:64,borderRadius:16,overflow:"hidden",margin:"0 auto 12px",border:`2px solid ${C.border}` }}>
              <img src={products[step-1].img} alt={products[step-1].name} style={{ width:"100%",height:"100%",objectFit:"cover" }} />
            </div>
            <h2 style={{ fontFamily:"'Fraunces',serif",fontSize:mob?18:22,fontWeight:600,marginBottom:4 }}>{products[step-1].name}</h2>
            <p style={{ fontSize:14,color:C.muted,marginBottom:4 }}>{products[step-1].qty}× ordered</p>
            <p style={{ fontSize:13,color:C.light,marginBottom:20 }}>How was the quality and freshness?</p>
            <div style={{ display:"flex",justifyContent:"center",marginBottom:24 }}>
              <StarRow value={productRatings[step-1]} onChange={(v) => setProductRating(step-1, v)} size={mob?28:32} />
            </div>
          </div>
        )}
        ...
        <div style={{ display:"flex",gap:10 }}>
          {step > 0 && (
            <button onClick={() => setStep(step-1)} style={{ flex:1,fontSize:14,fontWeight:600,color:C.muted,background:"rgba(0,0,0,0.04)",border:"none",padding:"13px",borderRadius:100,cursor:"pointer" }}>Back</button>
          )}
          <button onClick={handleNext} disabled={currentRating===0} style={{ flex:2,fontSize:14,fontWeight:700,color:"#FFF",background:currentRating===0?"#ccc":`linear-gradient(135deg,${C.accent},${C.accentL})`,border:"none",padding:"13px",borderRadius:100,cursor:currentRating===0?"not-allowed":"pointer",transition:"background 0.2s" }}>
            {isLast ? "Submit" : "Next →"}
          </button>
        </div>

        {/* Step label */}
        <div style={{ textAlign:"center",marginTop:12,fontSize:12,color:C.light }}>{step+1} of {totalSteps}</div>
      </div>
    </Overlay>
  );
}
